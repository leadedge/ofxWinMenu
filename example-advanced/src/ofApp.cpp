	...
	if(title == "Open") {
		...
		if(result.bSuccess) {
			...
			if (myImage.load(filePath)) {
				...
				MessageBoxA(NULL, "Could not load image", "Information", MB_OK);
			}
		}
	}
	if(title == "Save As") {
		...
		if(result.bSuccess) {
			...
			if(pos != string::npos && pos != 0 && filePath.substr(pos+1) == "jpg") {
				...
				MessageBoxA(NULL, "Needs a '.jpg' extension", "Information", MB_OK);
			}
		}
	}
	if(title == "Alert") {
...
void ofApp::draw() {
	...
	if(bShowInfo) {
		...
		sprintf_s(str, 256, " ' ' show/hide info : 'f' toggle full screen : RH click for options");
